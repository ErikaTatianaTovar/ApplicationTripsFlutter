...
import '../model/user_data.dart' as modelUser;
...
class CloudFirestoreApi {
  ...
  void updateUserData(modelUser.UserData userData) async {
    ...
    referenceUsers
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        _dataBase.collection(USERS).doc(userData.userId);
        ...
        return await reference.set({
          ...
          'email': userData.email,
          'photoUrl': userData.photoURL,
          'myPlaces': userData.myPlaces,
          'myFavoritePlaces': userData.myFavoritePlaces,
          'lastSigIn': DateTime.now() },
            SetOptions(merge: true));
         }
       }
  ...
  List<Place> buildPlaces(
      List<DocumentSnapshot> placesListSnapshot, modelUser.UserData userData) {
    List<Place> places = [];
    ...
    placesListSnapshot.forEach((DocumentSnapshot place) {
      ...
      if (usersLiked != null) {
        usersLiked.forEach((userLiked) {
          ...
          if (userReference.id == userData.userId) {
            isLiked = true;
          } else {
            isLiked = false;
          }
